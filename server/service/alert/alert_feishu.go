...
func PostToFS(title, text, Fsurl, userEmail string) string {
	open := "1"
	if open != "1" {
		global.GVA_LOG.Info("[feishu] PostToFS is closed")
		return "飞书接口未配置未开启状态,请先配置open-feishu为1"
	}
	RTstring := ""
	if strings.Contains(Fsurl, "/v2/") {
		RTstring = PostToFeiShuv2(title, text, Fsurl, "")
	} else {
		RTstring = PostToFeiShu(title, text, Fsurl)
	}
	return RTstring
}

func PostToFeiShu(title, text, Fsurl string) string {
	fmt.Println(title, text, Fsurl)
	...
	if err != nil {
		global.GVA_LOG.Error("[feishu] PostToFeiShu error", zap.Error(err))
	}
	result, err := ioutil.ReadAll(res.Body)
	fmt.Println("result", string(result))
	defer res.Body.Close()
	if err != nil {
		global.GVA_LOG.Error("[feishu] ReadAll error", zap.Error(err))
	}
...
func PostToFeiShuv2(title, text, Fsurl, userOpenId string) string {
	var color string
